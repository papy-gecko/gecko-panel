...
class ListFiles extends ListRecords
{
        ...
        return $table
            ...
            ->recordActions([
                ...
                Action::make('fm_download')
                    ...
                    ->action(function (File $file) {
                        if ($file->is_directory) {
                            $archiveName = $file->name . '_' . now()->format('YmdHis');
                            $archive = $this->getDaemonFileRepository()->compressFiles($this->path, [$file->name], $archiveName, 'zip');
                            $path = encode_path(join_paths($this->path, $archive['name']));
                        } else {
                            $path = encode_path(join_paths($this->path, $file->name));
                        }
                        $this->redirect(DownloadFiles::getUrl(['path' => $path]));
                    }),
                ActionGroup::make([
            ...
            ->toolbarActions([
                ...
                Action::make('open_terminal')
                    ->authorize(fn () => user()?->can(SubuserPermission::FileRead, $server))
                    ->hiddenLabel()
                    ->tooltip('Ouvrir le terminal ici')
                    ->icon(TablerIcon::Terminal)
                    ->color('gray')
                    ->extraAttributes(fn () => [
                        'x-on:click.prevent' => "navigator.clipboard.writeText('cd " . addslashes($this->path) . "').catch(()=>{}); window.open('https://terminal.lespotesagees.fr', '_blank'); new window.FilamentNotification().title('Terminal ouvert — Ctrl+V pour naviguer dans ce dossier').info().duration(5000).send();",
                    ])
                    ->url('#'),
            ]);
    }
    ...
    public function createFolderBatch(string $folderPath): bool
    {
        /** @var Server $server */
        $server = Filament::getTenant();
